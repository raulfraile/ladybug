...
class Ladybug {
    ...
    public static function getInstance() {
        return (self::$instance !== null) ? self::$instance : (self::$instance = new Ladybug()); 
    }
    ...
    private function dump_object($var) {
        ...
        $include_file = $this->getIncludePath($class_name, 'object');
    ...
    private function dump_resource($var) {
        ...
        $include_file = $this->getIncludePath($resource_type, 'resource');
    ...
    private function getIncludePath($name, $type = 'object') {
        ...
        if ($type == 'object') $path = __DIR__ . '/' . self::OBJECTS_FOLDER . '/' . str_replace(' ', '_', strtolower($name)) . '.php';
        elseif ($type == 'resource') $path = __DIR__ .'/' . self::RESOURCES_FOLDER . '/' . str_replace(' ', '_', strtolower($name)) . '.php';
    ...
    private function getIncludeClass($name, $type = 'object') {
        ...
        if ($type == 'object') $class = 'Ladybug_Objects_'.str_replace(' ', '_', ucwords($name));
        elseif ($type == 'resource') $class = 'Ladybug_Resources_'.str_replace(' ', '_', ucwords($name));
